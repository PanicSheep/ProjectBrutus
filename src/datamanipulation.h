...
enum DataType { Position_Score, Position_Score_PV, Position_FullScore, Edax };
...
struct CPositionScore
{
	...
	explicit CPositionScore(const CDataset_Position_Score&     that);
...
struct CPositionScoreDepthSelectivity
{
	unsigned long long P, O;
	signed char score;
	signed char depth;
	...
	CPositionScoreDepthSelectivity(unsigned long long P, unsigned long long O, signed char score) : P(P), O(O), score(score) {}
	
	explicit CPositionScoreDepthSelectivity(const CDataset_Position_Score&     that);
	explicit CPositionScoreDepthSelectivity(const CDataset_Position_Score_PV&  that);
	explicit CPositionScoreDepthSelectivity(const CDataset_Position_FullScore& that);
	explicit CPositionScoreDepthSelectivity(const CDataset_Edax&               that);
};
...
template<class T>
std::vector<T> read_vector(const std::string & filename, std::size_t size = 0xFFFFFFFFFFFFFFFFULL) { return DataManipulation::read_vector2<T>(filename, size); }
